...
public class PokerGameEngine {
    ...
    private void evaluateShowdown() {
        ...
        for (TableSeat seat : activePlayers) {
            // Note: Ensure your TableSeat class has a getter like getHoleCards() or getCards()
    ...
    public void addObserver(IGameEventListener observer) { this.observers.add(observer); }
    private void notifySeatOccupied(TableSeat newSeat) { observers.forEach(o -> o.onNewSeatOccupied(newSeat)); }
    ...
    private void notifyPlayerTurn(TableSeat actor, int amount) { observers.forEach(o -> o.onPlayerTurn(actor, amount)); }
    private void notifyPlayerAction(TableSeat actor, String actionType, int amount) { observers.forEach(o -> o.onPlayerAction(actor, actionType, amount)); }
    private void notifyHandResult(List<TableSeat> winners, HandResult winnerHand, int potSize) { observers.forEach(o -> o.onHandResult(winners, winnerHand, potSize)); }
}